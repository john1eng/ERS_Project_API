...
export class EmplReimbRepository implements CrudRepository<Reimb> {
    ...
    async update(updatedEmplReimb: Reimb): Promise<boolean> {
        ...
        try {
            ...
            // WIP: hacky fix since we need to make two DB calls
            let authorId = (await client.query('select ERS_USER_ID from ERS_USERS where USERNAME = $1', [updatedEmplReimb.AUTHOR_USERNAME])).rows[0].ers_user_id;
            console.log('authorid')
            let statusId = (await client.query('select REIMB_STATUS_ID from ERS_REIMBURSEMENT_STATUSES where REIMB_STATUS = $1', [updatedEmplReimb.REIMB_STATUS])).rows[0].reimb_status_id;
            console.log('statusid')
            let resolverId = (await client.query('select ERS_USER_ID from ERS_USERS where USERNAME = $1', [updatedEmplReimb.RESOLVER_USERNAME])).rows[0].ers_user_id;
            ...
            let sql = `update ERS_REIMBURSEMENT 
                    set AMOUNT = '${updatedEmplReimb.AMOUNT}', SUBMITTED = '${updatedEmplReimb.SUBMITTED}', RESOLVED = '${updatedEmplReimb.RESOLVED}', DESCRIPTION = '${updatedEmplReimb.DESCRIPTION}', RECEIPT = '${updatedEmplReimb.RECEIPT}', AUTHOR_ID = '${authorId}', RESOLVER_ID = '${resolverId}', REIMB_STATUS_ID = '${statusId}', REIMB_TYPE_ID = '${typeId}'
                    where REIMB_ID = $1`;
            console.log(sql)
